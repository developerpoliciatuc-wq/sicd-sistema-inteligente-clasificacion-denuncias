...
def procesar_archivo(
    ...
    if denuncia.direccion_hecho:
        try:
            ...
            logger.exception("Fallo geocodificación")

    # 5) Exportar para QGIS (Formulario automático)
    try:
        from src.services.qgis_exporter import exportar_para_qgis
        exportar_para_qgis(denuncia, repo_root)
    except Exception:
        logger.exception("Fallo exportación para QGIS")

    # 6) Guardar resultado y estadísticas
    dest = _write_and_stats(denuncia, motivo_revision or "OK")

    texto_final = texto
    if motivo_revision:
        return texto_final, denuncia, motivo_revision, dest
    
    return texto_final, denuncia, None, dest
